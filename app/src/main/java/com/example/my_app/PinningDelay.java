...
public class PinningDelay {
    private final Context context;
    private final String PREFS_NAME = "settings";
    private final String KEY_DELAY = "pinning_delay";

    public PinningDelay(Context context) {
    ...
    private void updateText(TextView valueText, int progress) {
        if (progress < 14) {
            float seconds = 1.0f + progress;
            valueText.setText(String.format(Locale.US, "%.1f sec", seconds));
        } else {
            valueText.setText("\u221E");
        }
    }
